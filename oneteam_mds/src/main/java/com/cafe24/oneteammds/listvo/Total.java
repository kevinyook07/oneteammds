package com.cafe24.oneteammds.listvo;

public class Total {

	private String dbCode;
	private String hospitalId;
	private String patientId;
	private String dName;
	private String dCode;
	private String dDate;
	private String pDoctor;
	private String adminId;
	private String drDate;
	
	private String mbCode;
	private String mmName;
	private String mmCode;
	private String miName;
	private String miCode;
	private String mVolume;
	private String mdUnit;
	private String mdNumber;
	private String mPeriod;
	private String mUsage;
	private String mdTime;
	private String mrDate;
	
	private String strCode;
	private String stiName;
	private String stiCode;
	private String stName;
	private String strValue;
	private String strrValue;
	private String stdTime;
	private String strDate;
	
	private String ptrCode;
	private String ptName;
	private String ptCode;
	private String ptrValue;
	private String ptdTime;
	private String ptrDate;
	
	private String itrCode;
	private String itName;
	private String itCode;
	private String itrValue;
	private String itdTime;
	private String itrDate;
	
	private String iidCode;
	private String liContent;
	private String iUrl;
	private String idName;
	private String ifdTime;
	private String iliDate;
	private String iidrDate;
	
	private String ftrCode;
	private String ftName;
	private String ftCode;
	private String ftrValue;
	private String ftdTime;
	private String ftrDate;
	
	private String sbCode;
	private String sName;
	private String sCode;
	private String aKind;
	private String sdTime;
	private String sbDate;
	
	private String aseCode;
	private String asemFactor;
	private String asemfCode;
	private String aseName;
	private String aseReaction;
	private String asedTime;
	private String aserDate;
	
	private String pibCode;
	private String piName;
	private String piCode;
	private String pivName;
	private String piBalance;
	private String piDate;
	private String pirDate;
	
	private String vscCode;
	private String vscHeight;
	private String vscWeight;
	private String bpMin;
	private String bpMax;
	private String bheat;
	private String vscCheck;
	private String vscrDate;
	
	private String scCode;
	private String sccName;
	private String scrDate;
	
	private String dcCode;
	private String dccName;
	private String dcrDate;
	
	private String lcidCode;
	private String lcidName;
	private String lcidpGroup;
	private String lcidtResult;
	private String lcidinOut;
	private String lcidArea;
	private String lcidoDate;
	private String lciddDate;
	private String lcidrDate;
	private String lcidDate;
	
	private String pName;

	public String getDbCode() {
		return dbCode;
	}

	public void setDbCode(String dbCode) {
		this.dbCode = dbCode;
	}

	public String getHospitalId() {
		return hospitalId;
	}

	public void setHospitalId(String hospitalId) {
		this.hospitalId = hospitalId;
	}

	public String getPatientId() {
		return patientId;
	}

	public void setPatientId(String patientId) {
		this.patientId = patientId;
	}

	public String getdName() {
		return dName;
	}

	public void setdName(String dName) {
		this.dName = dName;
	}

	public String getdCode() {
		return dCode;
	}

	public void setdCode(String dCode) {
		this.dCode = dCode;
	}

	public String getdDate() {
		return dDate;
	}

	public void setdDate(String dDate) {
		this.dDate = dDate;
	}

	public String getpDoctor() {
		return pDoctor;
	}

	public void setpDoctor(String pDoctor) {
		this.pDoctor = pDoctor;
	}

	public String getAdminId() {
		return adminId;
	}

	public void setAdminId(String adminId) {
		this.adminId = adminId;
	}

	public String getDrDate() {
		return drDate;
	}

	public void setDrDate(String drDate) {
		this.drDate = drDate;
	}

	public String getMbCode() {
		return mbCode;
	}

	public void setMbCode(String mbCode) {
		this.mbCode = mbCode;
	}

	public String getMmName() {
		return mmName;
	}

	public void setMmName(String mmName) {
		this.mmName = mmName;
	}

	public String getMmCode() {
		return mmCode;
	}

	public void setMmCode(String mmCode) {
		this.mmCode = mmCode;
	}

	public String getMiName() {
		return miName;
	}

	public void setMiName(String miName) {
		this.miName = miName;
	}

	public String getMiCode() {
		return miCode;
	}

	public void setMiCode(String miCode) {
		this.miCode = miCode;
	}

	public String getmVolume() {
		return mVolume;
	}

	public void setmVolume(String mVolume) {
		this.mVolume = mVolume;
	}

	public String getMdUnit() {
		return mdUnit;
	}

	public void setMdUnit(String mdUnit) {
		this.mdUnit = mdUnit;
	}

	public String getMdNumber() {
		return mdNumber;
	}

	public void setMdNumber(String mdNumber) {
		this.mdNumber = mdNumber;
	}

	public String getmPeriod() {
		return mPeriod;
	}

	public void setmPeriod(String mPeriod) {
		this.mPeriod = mPeriod;
	}

	public String getmUsage() {
		return mUsage;
	}

	public void setmUsage(String mUsage) {
		this.mUsage = mUsage;
	}

	public String getMdTime() {
		return mdTime;
	}

	public void setMdTime(String mdTime) {
		this.mdTime = mdTime;
	}

	public String getMrDate() {
		return mrDate;
	}

	public void setMrDate(String mrDate) {
		this.mrDate = mrDate;
	}

	public String getStrCode() {
		return strCode;
	}

	public void setStrCode(String strCode) {
		this.strCode = strCode;
	}

	public String getStiName() {
		return stiName;
	}

	public void setStiName(String stiName) {
		this.stiName = stiName;
	}

	public String getStiCode() {
		return stiCode;
	}

	public void setStiCode(String stiCode) {
		this.stiCode = stiCode;
	}

	public String getStName() {
		return stName;
	}

	public void setStName(String stName) {
		this.stName = stName;
	}

	public String getStrValue() {
		return strValue;
	}

	public void setStrValue(String strValue) {
		this.strValue = strValue;
	}

	public String getStrrValue() {
		return strrValue;
	}

	public void setStrrValue(String strrValue) {
		this.strrValue = strrValue;
	}

	public String getStdTime() {
		return stdTime;
	}

	public void setStdTime(String stdTime) {
		this.stdTime = stdTime;
	}

	public String getStrDate() {
		return strDate;
	}

	public void setStrDate(String strDate) {
		this.strDate = strDate;
	}

	public String getPtrCode() {
		return ptrCode;
	}

	public void setPtrCode(String ptrCode) {
		this.ptrCode = ptrCode;
	}

	public String getPtName() {
		return ptName;
	}

	public void setPtName(String ptName) {
		this.ptName = ptName;
	}

	public String getPtCode() {
		return ptCode;
	}

	public void setPtCode(String ptCode) {
		this.ptCode = ptCode;
	}

	public String getPtrValue() {
		return ptrValue;
	}

	public void setPtrValue(String ptrValue) {
		this.ptrValue = ptrValue;
	}

	public String getPtdTime() {
		return ptdTime;
	}

	public void setPtdTime(String ptdTime) {
		this.ptdTime = ptdTime;
	}

	public String getPtrDate() {
		return ptrDate;
	}

	public void setPtrDate(String ptrDate) {
		this.ptrDate = ptrDate;
	}

	public String getItrCode() {
		return itrCode;
	}

	public void setItrCode(String itrCode) {
		this.itrCode = itrCode;
	}

	public String getItName() {
		return itName;
	}

	public void setItName(String itName) {
		this.itName = itName;
	}

	public String getItCode() {
		return itCode;
	}

	public void setItCode(String itCode) {
		this.itCode = itCode;
	}

	public String getItrValue() {
		return itrValue;
	}

	public void setItrValue(String itrValue) {
		this.itrValue = itrValue;
	}

	public String getItdTime() {
		return itdTime;
	}

	public void setItdTime(String itdTime) {
		this.itdTime = itdTime;
	}

	public String getItrDate() {
		return itrDate;
	}

	public void setItrDate(String itrDate) {
		this.itrDate = itrDate;
	}

	public String getIidCode() {
		return iidCode;
	}

	public void setIidCode(String iidCode) {
		this.iidCode = iidCode;
	}

	public String getLiContent() {
		return liContent;
	}

	public void setLiContent(String liContent) {
		this.liContent = liContent;
	}

	public String getiUrl() {
		return iUrl;
	}

	public void setiUrl(String iUrl) {
		this.iUrl = iUrl;
	}

	public String getIdName() {
		return idName;
	}

	public void setIdName(String idName) {
		this.idName = idName;
	}

	public String getIfdTime() {
		return ifdTime;
	}

	public void setIfdTime(String ifdTime) {
		this.ifdTime = ifdTime;
	}

	public String getIliDate() {
		return iliDate;
	}

	public void setIliDate(String iliDate) {
		this.iliDate = iliDate;
	}

	public String getIidrDate() {
		return iidrDate;
	}

	public void setIidrDate(String iidrDate) {
		this.iidrDate = iidrDate;
	}

	public String getFtrCode() {
		return ftrCode;
	}

	public void setFtrCode(String ftrCode) {
		this.ftrCode = ftrCode;
	}

	public String getFtName() {
		return ftName;
	}

	public void setFtName(String ftName) {
		this.ftName = ftName;
	}

	public String getFtCode() {
		return ftCode;
	}

	public void setFtCode(String ftCode) {
		this.ftCode = ftCode;
	}

	public String getFtrValue() {
		return ftrValue;
	}

	public void setFtrValue(String ftrValue) {
		this.ftrValue = ftrValue;
	}

	public String getFtdTime() {
		return ftdTime;
	}

	public void setFtdTime(String ftdTime) {
		this.ftdTime = ftdTime;
	}

	public String getFtrDate() {
		return ftrDate;
	}

	public void setFtrDate(String ftrDate) {
		this.ftrDate = ftrDate;
	}

	public String getSbCode() {
		return sbCode;
	}

	public void setSbCode(String sbCode) {
		this.sbCode = sbCode;
	}

	public String getsName() {
		return sName;
	}

	public void setsName(String sName) {
		this.sName = sName;
	}

	public String getsCode() {
		return sCode;
	}

	public void setsCode(String sCode) {
		this.sCode = sCode;
	}

	public String getaKind() {
		return aKind;
	}

	public void setaKind(String aKind) {
		this.aKind = aKind;
	}

	public String getSdTime() {
		return sdTime;
	}

	public void setSdTime(String sdTime) {
		this.sdTime = sdTime;
	}

	public String getSbDate() {
		return sbDate;
	}

	public void setSbDate(String sbDate) {
		this.sbDate = sbDate;
	}

	public String getAseCode() {
		return aseCode;
	}

	public void setAseCode(String aseCode) {
		this.aseCode = aseCode;
	}

	public String getAsemFactor() {
		return asemFactor;
	}

	public void setAsemFactor(String asemFactor) {
		this.asemFactor = asemFactor;
	}

	public String getAsemfCode() {
		return asemfCode;
	}

	public void setAsemfCode(String asemfCode) {
		this.asemfCode = asemfCode;
	}

	public String getAseName() {
		return aseName;
	}

	public void setAseName(String aseName) {
		this.aseName = aseName;
	}

	public String getAseReaction() {
		return aseReaction;
	}

	public void setAseReaction(String aseReaction) {
		this.aseReaction = aseReaction;
	}

	public String getAsedTime() {
		return asedTime;
	}

	public void setAsedTime(String asedTime) {
		this.asedTime = asedTime;
	}

	public String getAserDate() {
		return aserDate;
	}

	public void setAserDate(String aserDate) {
		this.aserDate = aserDate;
	}

	public String getPibCode() {
		return pibCode;
	}

	public void setPibCode(String pibCode) {
		this.pibCode = pibCode;
	}

	public String getPiName() {
		return piName;
	}

	public void setPiName(String piName) {
		this.piName = piName;
	}

	public String getPiCode() {
		return piCode;
	}

	public void setPiCode(String piCode) {
		this.piCode = piCode;
	}

	public String getPivName() {
		return pivName;
	}

	public void setPivName(String pivName) {
		this.pivName = pivName;
	}

	public String getPiBalance() {
		return piBalance;
	}

	public void setPiBalance(String piBalance) {
		this.piBalance = piBalance;
	}

	public String getPiDate() {
		return piDate;
	}

	public void setPiDate(String piDate) {
		this.piDate = piDate;
	}

	public String getPirDate() {
		return pirDate;
	}

	public void setPirDate(String pirDate) {
		this.pirDate = pirDate;
	}

	public String getVscCode() {
		return vscCode;
	}

	public void setVscCode(String vscCode) {
		this.vscCode = vscCode;
	}

	public String getVscHeight() {
		return vscHeight;
	}

	public void setVscHeight(String vscHeight) {
		this.vscHeight = vscHeight;
	}

	public String getVscWeight() {
		return vscWeight;
	}

	public void setVscWeight(String vscWeight) {
		this.vscWeight = vscWeight;
	}

	public String getBpMin() {
		return bpMin;
	}

	public void setBpMin(String bpMin) {
		this.bpMin = bpMin;
	}

	public String getBpMax() {
		return bpMax;
	}

	public void setBpMax(String bpMax) {
		this.bpMax = bpMax;
	}

	public String getBheat() {
		return bheat;
	}

	public void setBheat(String bheat) {
		this.bheat = bheat;
	}

	public String getVscCheck() {
		return vscCheck;
	}

	public void setVscCheck(String vscCheck) {
		this.vscCheck = vscCheck;
	}

	public String getVscrDate() {
		return vscrDate;
	}

	public void setVscrDate(String vscrDate) {
		this.vscrDate = vscrDate;
	}

	public String getScCode() {
		return scCode;
	}

	public void setScCode(String scCode) {
		this.scCode = scCode;
	}

	public String getSccName() {
		return sccName;
	}

	public void setSccName(String sccName) {
		this.sccName = sccName;
	}

	public String getScrDate() {
		return scrDate;
	}

	public void setScrDate(String scrDate) {
		this.scrDate = scrDate;
	}

	public String getDcCode() {
		return dcCode;
	}

	public void setDcCode(String dcCode) {
		this.dcCode = dcCode;
	}

	public String getDccName() {
		return dccName;
	}

	public void setDccName(String dccName) {
		this.dccName = dccName;
	}

	public String getDcrDate() {
		return dcrDate;
	}

	public void setDcrDate(String dcrDate) {
		this.dcrDate = dcrDate;
	}

	public String getLcidCode() {
		return lcidCode;
	}

	public void setLcidCode(String lcidCode) {
		this.lcidCode = lcidCode;
	}

	public String getLcidName() {
		return lcidName;
	}

	public void setLcidName(String lcidName) {
		this.lcidName = lcidName;
	}

	public String getLcidpGroup() {
		return lcidpGroup;
	}

	public void setLcidpGroup(String lcidpGroup) {
		this.lcidpGroup = lcidpGroup;
	}

	public String getLcidtResult() {
		return lcidtResult;
	}

	public void setLcidtResult(String lcidtResult) {
		this.lcidtResult = lcidtResult;
	}

	public String getLcidinOut() {
		return lcidinOut;
	}

	public void setLcidinOut(String lcidinOut) {
		this.lcidinOut = lcidinOut;
	}

	public String getLcidArea() {
		return lcidArea;
	}

	public void setLcidArea(String lcidArea) {
		this.lcidArea = lcidArea;
	}

	public String getLcidoDate() {
		return lcidoDate;
	}

	public void setLcidoDate(String lcidoDate) {
		this.lcidoDate = lcidoDate;
	}

	public String getLciddDate() {
		return lciddDate;
	}

	public void setLciddDate(String lciddDate) {
		this.lciddDate = lciddDate;
	}

	public String getLcidrDate() {
		return lcidrDate;
	}

	public void setLcidrDate(String lcidrDate) {
		this.lcidrDate = lcidrDate;
	}

	public String getLcidDate() {
		return lcidDate;
	}

	public void setLcidDate(String lcidDate) {
		this.lcidDate = lcidDate;
	}

	public String getpName() {
		return pName;
	}

	public void setpName(String pName) {
		this.pName = pName;
	}

	@Override
	public String toString() {
		return "Total [dbCode=" + dbCode + ", hospitalId=" + hospitalId + ", patientId=" + patientId + ", dName="
				+ dName + ", dCode=" + dCode + ", dDate=" + dDate + ", pDoctor=" + pDoctor + ", adminId=" + adminId
				+ ", drDate=" + drDate + ", mbCode=" + mbCode + ", mmName=" + mmName + ", mmCode=" + mmCode
				+ ", miName=" + miName + ", miCode=" + miCode + ", mVolume=" + mVolume + ", mdUnit=" + mdUnit
				+ ", mdNumber=" + mdNumber + ", mPeriod=" + mPeriod + ", mUsage=" + mUsage + ", mdTime=" + mdTime
				+ ", mrDate=" + mrDate + ", strCode=" + strCode + ", stiName=" + stiName + ", stiCode=" + stiCode
				+ ", stName=" + stName + ", strValue=" + strValue + ", strrValue=" + strrValue + ", stdTime=" + stdTime
				+ ", strDate=" + strDate + ", ptrCode=" + ptrCode + ", ptName=" + ptName + ", ptCode=" + ptCode
				+ ", ptrValue=" + ptrValue + ", ptdTime=" + ptdTime + ", ptrDate=" + ptrDate + ", itrCode=" + itrCode
				+ ", itName=" + itName + ", itCode=" + itCode + ", itrValue=" + itrValue + ", itdTime=" + itdTime
				+ ", itrDate=" + itrDate + ", iidCode=" + iidCode + ", liContent=" + liContent + ", iUrl=" + iUrl
				+ ", idName=" + idName + ", ifdTime=" + ifdTime + ", iliDate=" + iliDate + ", iidrDate=" + iidrDate
				+ ", ftrCode=" + ftrCode + ", ftName=" + ftName + ", ftCode=" + ftCode + ", ftrValue=" + ftrValue
				+ ", ftdTime=" + ftdTime + ", ftrDate=" + ftrDate + ", sbCode=" + sbCode + ", sName=" + sName
				+ ", sCode=" + sCode + ", aKind=" + aKind + ", sdTime=" + sdTime + ", sbDate=" + sbDate + ", aseCode="
				+ aseCode + ", asemFactor=" + asemFactor + ", asemfCode=" + asemfCode + ", aseName=" + aseName
				+ ", aseReaction=" + aseReaction + ", asedTime=" + asedTime + ", aserDate=" + aserDate + ", pibCode="
				+ pibCode + ", piName=" + piName + ", piCode=" + piCode + ", pivName=" + pivName + ", piBalance="
				+ piBalance + ", piDate=" + piDate + ", pirDate=" + pirDate + ", vscCode=" + vscCode + ", vscHeight="
				+ vscHeight + ", vscWeight=" + vscWeight + ", bpMin=" + bpMin + ", bpMax=" + bpMax + ", bheat=" + bheat
				+ ", vscCheck=" + vscCheck + ", vscrDate=" + vscrDate + ", scCode=" + scCode + ", sccName=" + sccName
				+ ", scrDate=" + scrDate + ", dcCode=" + dcCode + ", dccName=" + dccName + ", dcrDate=" + dcrDate
				+ ", lcidCode=" + lcidCode + ", lcidName=" + lcidName + ", lcidpGroup=" + lcidpGroup + ", lcidtResult="
				+ lcidtResult + ", lcidinOut=" + lcidinOut + ", lcidArea=" + lcidArea + ", lcidoDate=" + lcidoDate
				+ ", lciddDate=" + lciddDate + ", lcidrDate=" + lcidrDate + ", lcidDate=" + lcidDate + ", pName="
				+ pName + "]";
	}
	
}
